...
Public Class dbtable
    ...
    Public Function changeFieldValue(ByVal originalValue As String, ByRef newValue As String) As Boolean
        Try
            Me._fields(originalValue) = newValue
            Return True
    ...
    End Sub
    Private Function findByFkField(ByVal id As String) As Boolean
        For Each fk As dbfk In Me._foreignKey.Values
            If fk.FOREIGNKEY = id Then
                Return True
            End If
        Next
        Return False
    ...
    Public Function et() As Boolean

        Dim allFlag As Boolean = True
        If Me._primaryKey.Keys.Count = 2 Then
            For Each pk As String In Me._primaryKey.Values
                If Not findByFkField(pk) Then
                    allFlag = allFlag And False
                End If
            Next
        End If
        Return (Not allFlag)
    ...
    Public Function getAllForeignKeysInPrimaryKey2() As String()
        Dim res As New List(Of String)
        For Each fkey As dbfk In Me._foreignKey.Values
            For Each pkey As String In Me._primaryKey.Values
                If pkey = fkey.FOREIGNKEY Then
                    res.Add(fkey.FOREIGNKEY)
                    Exit For
                End If
            Next
        Next
